    ...
    <nav className="sticky top-0 z-50 w-full bg-background/80 backdrop-blur-md border-b-2 border-primary/20 px-4 py-2">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        <Link href="/dashboard" className="flex items-center gap-3 group">
          <div className="relative w-12 h-12 rounded-xl overflow-hidden border-2 border-primary group-hover:glow-primary transition-all bg-white">
            <Image 
              src={institutionLogo} 
              alt={institutionName} 
              fill 
        ...
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex items-center bg-primary/5 px-4 py-1.5 rounded-full border-2 border-primary/20 shadow-sm">
            <Trophy className="w-4 h-4 text-accent mr-2" />
            <span className="font-black text-primary text-sm tabular-nums">
              {userData?.currentPoints || 0} <span className="text-[10px] opacity-60 ml-1">PTS</span>
            </span>
          </div>

          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="relative h-10 w-10 rounded-full border-2 border-primary p-0 ring-offset-background transition-all hover:scale-110 active:scale-95">
